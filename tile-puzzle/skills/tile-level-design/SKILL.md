...
- `n_types` — distinct normal `tile_id` + **1 if any special is present** (bonus 1001 & mission 1002 collapse to ONE bucket — a mixed mission+bonus level adds +1, not +2). `is_mystery` — 1 if any stone is mystery (`o:[0]` or legacy `m:true`).
...
| **BONUS `1001`** (round) / **MISSION `1002`** (square) | a NON-match-3 **cover** that AUTO-CLEARS when its footprint is clear on top. **Direction C:** specials are ADDED as interstitial covers over a COMPLETE ÷3 normal board — they NEVER consume a match-3 cell (normals alone stay ÷3). Footprint = **2×2 or 3×3**, encoded by `s`: MISSION `0.7`=2×2 / `1.0`=3×3; BONUS `0.9`=2×2 / `1.4`=3×3 (collision half 1.0 / 1.5 — shared with player + solver). Placement enforces the 7 rules (§5.1 spec): ≥1 normal **above** (covered-at-start) + ≥1 **below**; whole footprint **within layout bbox**; **never overlaps a STACK column**; **distinct (x,y)** — no two specials share a coordinate even on different layers; **overlapping specials need a NORMAL tile on a layer between them** (anti chain-reveal — not just distinct layers); **even layer spread** (farthest-first, count diff ≤1). Placed offset/straddling. | added **after a solvable normal board** (built in one pass) | `scripts/reserve_special.py --bonus N --mission M` (no `--*-cover` → **auto-mix** 2×2+3×3; force with `--mission-cover/--bonus-cover 2x2\|3x3`; explicit counts `--mission-2x2/--mission-3x3` etc.; `--size` overrides `s`) |
| **MYSTERY `o:[0]`** (legacy `m:true`) | a NORMAL match-3 tile that is FACE-DOWN — colour FIXED at design time. It stays COVERED on the board **even when pickable**: the player picks it BLIND and its real colour is revealed only once it lands in the TRAY (distinct from CLOUD, which reveals on-board when uncovered). Plays as a normal tile (÷3 unchanged) → no effect on geometry/balance/solvability. Placement: **EVEN across layers, ≤2 per layer**, only over layers with ≥1 real normal tile. Count is **context-aware**: **5** alone / **4** with Mission/Bonus / **3** with Cloud. `o`: **0=mystery, 1=cloud** (`m:true` is the OLD marker, still read) | **after tiles** | `scripts/add_special_cells.py --mystery N` (omit N → context-aware 5/4/3; emits `o:[0]`; `--mark` is a kept alias) |
| **CLOUD `o:[1]`** | a NORMAL match-3 tile covered by the mystery cover art (fills the WHOLE tile); the cover clears MISSION-STYLE (the instant nothing on a higher layer overlaps it = when it becomes pickable), revealing the real face — only the cover layer clears, the tile stays. Plays as a normal tile → NO effect on solvability (solver ignores `o`). Placed on the BOTTOM layer(s) 0-1 only (NEVER the top — must start covered), **~15-20% of tiles**. Symmetry is **HYBRID**: symmetric orbits first (aesthetic where geometry allows), then top up from any covered+visible cell to hit target — so it **never leaves 0 clouds** when candidates exist (kills the old hard-symmetry gate). Candidate cells must be COVERED **and VISIBLE (peek)** — no tile directly on top — so cloud levels REQUIRE a **STAGGERED layout** (gen-layout default `uniform_stagger`, §9); a fully COLUMNAR layout hides every bottom cell → 0 candidates (only true 0 case). `o` value: **1=cloud, 0=mystery** | **after tiles** | `scripts/add_cloud.py` (default 18%, `--cloud-pct`/`--cloud N`, `--axis auto`, `--layers 0,1`) |
...
LAST, after the level is already solvable, with `add_special_cells.py --mystery N` (default context-aware
5/4/3 = alone / +special / +cloud; even ≤2/layer; emits `o:[0]`; legacy `m:true` still read).
