...
- [Solver infrastructure](solver_infrastructure.md) — verify_smart_v3 / solve_path / solve_dispatch.solve_any are the go-to scripts; use `gen_pattern.py --pattern N` (SKILL.md §4), not find_* templates, to generate levels
...
- [5 Level Design Patterns](level_design_patterns.md) — 5 patterns, ORIGINAL research numbering (trap ẩn, easy-top, hybrid custom, 90% fail, clear-50%); see SKILL.md §4 for the canonical 6-pattern `gen_pattern.py` numbering/mapping
...
- [Greedy vs exact solving](greedy_vs_exact.md) — 4 mechanisms that touch "greedy vs exact" (playout() stochastic evaluator, min_safe_choices exact per-step probe, solver-internal atomic-triple collapse, deprecated TileSolver.analyze) — decision table, don't conflate; `analyze_level.py --solve-profile` is where the joint conclusion is persisted
- [Solver pruning history](solver_pruning_history.md) — NP-complete, so no polynomial exact algorithm; atomic-triple collapse and tray-submultiset dominance are PROVEN UNSOUND for existence queries (winnable/min_safe_choices); POR local-independence is theoretically sound but unimplemented; read before attempting any new solve_v3/winnable pruning
